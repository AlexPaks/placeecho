    ...
    y: 0
      ...
      <div className="relative z-10">
        <div className="flex justify-between items-start mb-4">
          ...
          <button onClick={onRefresh} disabled={loading} className={`p-2 rounded-full hover:bg-stone-100 transition-colors ${loading ? 'animate-spin' : ''}`} aria-label="Refresh location">
        ...
        {loading ? <div className="h-16 flex items-center justify-center">
          ...
          </div> : error ? <div className="text-red-500 text-sm bg-red-50 p-3 rounded-lg">
            {error}
            <button onClick={onRefresh} className="block mt-2 text-red-700 underline text-xs">
              Try again
            </button>
          </div> : location ? <div>
          ...
          </div> : <div className="text-stone-500 text-sm italic">
